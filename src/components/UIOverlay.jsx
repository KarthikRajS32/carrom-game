...
import { GamePhase, CoinType } from '../config/types.js';
import { TURN_TIME_LIMIT } from '../config/constants.js';
    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between font-sans">
      ...
      {winner && (
        <div className="absolute inset-0 bg-stone-950/90 flex items-center justify-center pointer-events-auto z-50 backdrop-blur-md p-4">
           <div className="bg-stone-900 p-6 sm:p-12 rounded-3xl border-4 border-amber-500 text-center shadow-[0_0_50px_rgba(251,191,36,0.3)] animate-fade-in max-w-xs sm:max-w-sm w-full">
              <div className="text-amber-500 text-xs sm:text-sm font-bold uppercase tracking-[0.3em] mb-2">Victory</div>
              <h2 className="text-3xl sm:text-5xl font-black text-white mb-4 sm:mb-6 italic tracking-tighter">{winner.toUpperCase()}</h2>
              <div className="bg-stone-800 p-3 sm:p-6 rounded-2xl mb-4 sm:mb-8 space-y-2 sm:space-y-3">
                 ...
                 <div className="flex justify-between items-center">
                    ...
                    <span className="text-xl sm:text-2xl font-black text-amber-400">{score.white}</span>
                 </div>
                 <div className="flex justify-between items-center">
                    <span className="text-white font-medium text-sm sm:text-base">Black Team</span>
                    <span className="text-xl sm:text-2xl font-black text-amber-400">{score.black}</span>
                 </div>
              </div>
              <button 
                onClick={onMenu}
                className="bg-amber-500 hover:bg-amber-400 active:bg-amber-600 text-stone-900 px-6 sm:px-12 py-3 sm:py-4 rounded-xl font-black w-full transition-all shadow-lg text-sm sm:text-lg uppercase tracking-tight active:scale-95"
              >
                Return to Menu
              </button>
           </div>
