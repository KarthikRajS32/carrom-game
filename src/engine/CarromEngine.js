...
import { resolveCollision, distance, normalize, scale, subtract, dot } from '../utils/physics.js';

export class CarromEngine {
  constructor(canvas, settings, onGameStateChange, onNetworkSync) {
    this.canvas = canvas;
    ...
    this.winSound = new Audio('https://assets.mixkit.co/sfx/preview/mixkit-completion-of-a-level-2063.mp3');
  ...
  initPatterns() {
    const createPatternCanvas = (color, isDark) => {
  ...
  update() {
    ...
    if (this.phase === GamePhase.SHOOTING) {
      ...
      for (let step = 0; step < iterations; step++) {
        ...
        const pocketInset = POCKET_RADIUS + 8;
        const pockets = [
          { x: CUSHION_WIDTH + pocketInset, y: CUSHION_WIDTH + pocketInset },
          { x: BOARD_SIZE - CUSHION_WIDTH - pocketInset, y: CUSHION_WIDTH + pocketInset },
          { x: CUSHION_WIDTH + pocketInset, y: BOARD_SIZE - CUSHION_WIDTH - pocketInset },
          { x: BOARD_SIZE - CUSHION_WIDTH - pocketInset, y: BOARD_SIZE - CUSHION_WIDTH - pocketInset }
        ];
  ...
  handleTurnEnd() {
    ...
    if (this.score.winner) {
      // Play winning sound and create celebration particles
      if (this.settings.soundEnabled) {
      ...
      for (let i = 0; i < 20; i++) {
        this.particles.push({
          x: BOARD_SIZE / 2 + (Math.random() - 0.5) * 200,
          y: BOARD_SIZE / 2 + (Math.random() - 0.5) * 200,
      ...
      this.notifyUI(`🎉 ${this.score.winner} Wins! 🎉`);
      return;
    }
  ...
  drawBoardDesign() {
    ...
    // Corner Pockets - Real carrom board style with rounded corners and nets
    const pocketInset = POCKET_RADIUS + 2; // Distance from corner
    const corners = [
      [CUSHION_WIDTH + pocketInset, CUSHION_WIDTH + pocketInset], 
      [BOARD_SIZE - CUSHION_WIDTH - pocketInset, CUSHION_WIDTH + pocketInset], 
      [CUSHION_WIDTH + pocketInset, BOARD_SIZE - CUSHION_WIDTH - pocketInset], 
      [BOARD_SIZE - CUSHION_WIDTH - pocketInset, BOARD_SIZE - CUSHION_WIDTH - pocketInset]
    ];
    
    corners.forEach(([x,y]) => {
      // Outer pocket ring (wood)
      this.ctx.fillStyle = COLORS.BOARD_BORDER;
      this.ctx.beginPath();
      this.ctx.arc(x, y, POCKET_RADIUS + 2, 0, Math.PI * 2);
      this.ctx.fill();
      
      // Main pocket hole
      this.ctx.fillStyle = COLORS.POCKET;
      this.ctx.beginPath();
      this.ctx.arc(x, y, POCKET_RADIUS, 0, Math.PI * 2);
      this.ctx.fill();
      ...
      // Net pattern inside pocket
      this.ctx.strokeStyle = 'rgba(100, 100, 100, 0.4)';
      this.ctx.lineWidth = 1;
      for(let i = -POCKET_RADIUS; i <= POCKET_RADIUS; i += 8) {
        this.ctx.beginPath();
        this.ctx.moveTo(x + i, y - Math.sqrt(POCKET_RADIUS*POCKET_RADIUS - i*i));
        this.ctx.lineTo(x + i, y + Math.sqrt(POCKET_RADIUS*POCKET_RADIUS - i*i));
        this.ctx.stroke();
        
        this.ctx.beginPath();
        this.ctx.moveTo(x - Math.sqrt(POCKET_RADIUS*POCKET_RADIUS - i*i), y + i);
        this.ctx.lineTo(x + Math.sqrt(POCKET_RADIUS*POCKET_RADIUS - i*i), y + i);
        this.ctx.stroke();
      }
    });

    // Baselines and Circles
    const drawSideDesign = (angle) => {
      this.ctx.save();
